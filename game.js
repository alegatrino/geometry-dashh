class GeometryDash {
    ...
    draw() {
        ...
        // РОТ (анимация)
        this.ctx.fillStyle = '#000000';
        const mouthHeight = this.player.isJumping ? 3 : 6;
        this.ctx.fillRect(-this.player.width/8, this.player.height/8, this.player.width/4, mouthHeight);
    ...
    gameOver() {
        ...
        if (finalScore) finalScore.textContent = `Счёт: ${this.score}`;
        if (gameContainer) {
            gameContainer.classList.remove('playing');
        }

        this.playSound('gameover');
        this.createParticleEffect(this.player.x, this.player.y, 30, '#FF0000');
    }
    ...
    shareScore() {
        if (navigator.share) {
            navigator.share({
                title: 'Geometry Dash - Technical Edition',
                text: `Я набрал ${this.score} очков в Geometry Dash! Попробуй побить мой рекорд!`,
